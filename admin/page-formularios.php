...
function sdt_render_formularios_page() {
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["sdt_save_forms_nonce"]) && wp_verify_nonce($_POST["sdt_save_forms_nonce"], "sdt_save_forms_action")) {
        if (current_user_can("manage_options")) {
            ...
            update_option('sdt_seo_avancado_template', ['sdt_seo_template_layout' => sanitize_text_field($_POST['sdt_seo_template_layout'] ?? '')]);
            echo '<div class="notice notice-success is-dismissible"><p>' . __("Formulários salvos com sucesso.", "sdt-domain") . '</p></div>';
        }
    }
    ?>
    ...
    <div class="wrap">
        ...
        <p><?php _e("Associe formulários específicos para cada produto, se necessário.", "sdt-domain"); ?></p>
        ...
        <form method="post">
            ...
            <table class="widefat striped">
                <thead>
                    <tr>
                        <th><?php _e("Usar", "sdt-domain"); ?></th>
                        <th><?php _e("Produto", "sdt-domain"); ?></th>
                        <th><?php _e("Formulário CF7", "sdt-domain"); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!empty($produtos)): ?>
                        ...
                        ?>
                            <tr>
                                <td><input type="checkbox" name="sdt_product_use[<?php echo $produto_esc; ?>]" <?php checked(isset($form_map[$produto])); ?>></td>
                                <td><?php echo esc_html($produto); ?></td>
                                <td>
                                    <select name="sdt_product_cf7_id[<?php echo $produto_esc; ?>]">
                                        <option value="0"><?php _e("-- Usar padrão --", "sdt-domain"); ?></option>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="3"><?php _e("Nenhum produto disponível.", "sdt-domain"); ?></td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
            ...
            <p class="submit">
                <?php submit_button(__('Salvar', 'sdt-domain')); ?>
            </p>
        </form>
    </div>
<?php } ?>
